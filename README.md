...
庆事通用版本化知识、证据链、确定性 Workflow 和受控 Agent Runtime，把“找到答案”推进到“按边界完成正确的下一步”。当前公开基线以脱敏、重写和虚拟化资料以及 A1 活动场地申请为深度业务主线，形成可运行、可回放、可验证的本地交付基线；后续目标是沿同一架构逐步接入更多经过授权的资料、业务事项和服务入口。
...
[项目上下文](CONTEXT.md) · [产品简报](PRODUCT.md) · [架构说明](docs/architecture.md) · [数据边界](docs/data-boundary.md) · [本地验证](docs/verification.md) · [贡献指南](CONTRIBUTING.md)
...
它们用于验证业务结构，不用于证明任何学校当前办理规则。所有状态变化只写入本地演示数据库，不能产生真实提交、审批或回执。公开仓库中的内容只能作为 `public_rewrite` 或 `virtual_design` 使用；真实观察资料仍属于本地研究边界。
...
公开仓库不保存真实学校页面、公告正文、登录后内容、截图、接口响应或个人记录。对于公开演示，进入 Publication 的内容只有脱敏重写结果和明确标注的虚拟设计。

知识内容在项目中分成三类：

| 标记 | 含义 | 使用规则 |
|---|---|---|
| local_observation | 本地研究中的真实公开或授权观察 | 只留在维护者控制的本地资料库 |
| public_rewrite | 删除真实组织、入口和可反推细节后的机制描述 | 可以进入公开文档或 Fixture |
| virtual_design | 为演示和评测设计的模拟公告、条款、地点、角色和状态 | 不冒充官方原文，必须保持虚拟边界 |

### 知识分层和血缘

RAG 的知识不是一个无法解释的文本集合，而是从来源到消费版本的可追踪链路：

- 本地研究层保留来源、授权、版本、定位和观察记录；它不进入公开 Git；
...
这套血缘既服务准确性，也服务公开治理：真实观察只能留在本地，公开演示只消费可公开的重写结果和虚拟设计。
...
当前公开 R0 是一个可复核的模拟基线，不是学校生产系统的缩小版。下一阶段不会因为“有 Agent”就覆盖所有校园业务。每个方向都要先证明三件事：普通搜索、筛选或固定表单哪里不够；Agent 能减少什么可测成本或错误；失败风险是否可控。真实校园资料仍只在本地研究；公开产品资料继续使用脱敏、重写和虚拟化内容。
...
真实资料只用于本地研究和经过授权的集成；公开仓库继续使用抽象机制、脱敏重写和虚拟数据。未来知识库可以覆盖公告、条款、材料、评奖、学分和流程等问题，但不能把真实个人数据或未经授权的内部页面直接放入公开仓库。
